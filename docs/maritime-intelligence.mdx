...
| `/ais/snapshot` | Merged AIS snapshot | Relay shared secret |
...
- `BARENTSWATCH_CLIENT_ID` / `BARENTSWATCH_CLIENT_SECRET` - free Norwegian terrestrial, offshore, and satellite AIS enrichment
- `OPENSKY_CLIENT_ID` - OAuth2 client ID
- `OPENSKY_CLIENT_SECRET` - OAuth2 client secret

The web API accepts `GFW_API_TOKEN` and proxies Global Fishing Watch vector
tiles without exposing the token to browsers. The Ships layer overlays delayed
global AIS presence (about 96 hours) and unmatched Sentinel-1 SAR vessel
detections (about five days) when this token is configured.
...
The relay keeps a bounded in-memory vessel snapshot, merging AISStream with
BarentsWatch while preferring recent AISStream observations for duplicate MMSIs.
